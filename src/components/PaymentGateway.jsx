...
function PaymentGateway({ cart, setPage, onProcess }) {
  ...
  // rawValue = hanya angka (tanpa titik)
  const [rawValue, setRawValue] = useState("");
  const [displayValue, setDisplayValue] = useState("");

  const handleChange = (e) => {
    const cleaned = e.target.value.replace(/[^\d]/g, ''); // hanya angka
    setRawValue(cleaned);

    const formatted = new Intl.NumberFormat('id-ID').format(cleaned);
  ...
  const cashParsed = parseInt(rawValue || '0', 10);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-white border rounded-2xl p-4 shadow-md space-y-3 flex flex-col justify-between">
            ...
            <div className="text-sm text-gray-700 space-y-1">
              ...
              <p>Amount: Rp.{cashParsed.toLocaleString('id-ID')}</p>
          ...
          <div className="bg-white border rounded-2xl p-4 shadow-md space-y-3">
            ...
            <img src="qr-code.jpg" alt="QR Code" className="w-32 h-32 mx-auto rounded-lg border" />
            <button
              className="bg-green-600 text-white text-sm px-4 py-2 rounded-xl font-semibold hover:bg-green-700 transition-colors w-full"
              onClick={() => {
                onProcess(customerName, 'QRIS');
                localStorage.removeItem('customerName');
                localStorage.removeItem('cart');
              }}
            >
